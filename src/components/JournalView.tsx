...
export const JournalIndex: React.FC<JournalIndexProps> = ({ articles, onOpenArticle }) => {
  ...
  return (
    <div className="pb-24">
      ...
      <section className="mx-auto max-w-[1400px] px-6 pt-12 md:px-8 md:pt-16">
        {/* Lead story — set apart by layout, not by chrome. The masthead above
            is already the biggest thing on the page; a headline at display size
            down here would just fight it. */}
        <button
          type="button"
        ...
        >
          <div className="grid items-center gap-6 md:grid-cols-2 md:gap-12">
            <div className="aspect-[16/10] overflow-hidden rounded-card bg-canvas">
              <img
                src={lead.image}
                alt=""
            ...
            <div>
              ...
              <h2 className="mt-3 text-title text-ink font-semibold md:text-title-lg">{lead.title}</h2>
              <p className="mt-3 text-body text-ink-secondary">{lead.dek}</p>
              <span className="mt-5 inline-flex items-center gap-1 text-footnote font-semibold text-link group-hover:underline">
        ...
        {/* Everything else */}
        <div className="mt-14 grid grid-cols-1 gap-x-6 gap-y-10 border-t border-hairline-soft pt-12 sm:grid-cols-2 lg:grid-cols-3">
          {rest.map((article) => (
            <button
              key={article.slug}
              type="button"
            ...
            >
              ...
              <div className="flex flex-1 flex-col pt-4">
                <p className="eyebrow text-ink-tertiary">{meta(article)}</p>
                <h3 className="mt-2 text-lead text-ink font-semibold group-hover:underline">{article.title}</h3>
...
export const ArticleView: React.FC<ArticleViewProps> = ({
  ...
  <article className="pb-24">
    ...
    {related.length > 0 && (
      <section className="mx-auto max-w-[1400px] px-6 pt-20 md:px-8">
        <h2 className="mb-8 border-t border-hairline-soft pt-8 text-title-sm text-ink font-semibold">
          Keep reading
        </h2>
        <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-3">
          {related.map((item) => (
            ...
            >
              ...
              <div className="pt-4">
                <p className="eyebrow text-ink-tertiary">{meta(item)}</p>
                <h3 className="mt-2 text-body text-ink font-semibold group-hover:underline">{item.title}</h3>
              </div>
            </button>
          ))}
        </div>
